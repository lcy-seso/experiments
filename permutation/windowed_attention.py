...
class RelPositionalWindowEmbedding(nn.Module):
    ...
    def forward(self, pos, x):
        ...
        relpos = (
            F.pad(pos,
                  (self.w, ) * 2).unfold(-1, seqlen, 1) - pos.unsqueeze(-2))
        sizes = relpos.size()
        sinusoid_inp = self.inv_freq.type_as(x) * relpos.unsqueeze(-1)
        embed = torch.stack(
            (sinusoid_inp.sin(), sinusoid_inp.cos()), dim=-1).view(
                *sizes, self.embedding_size)
        return embed
...
class MultiDimWindowAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.bias_r_w = self._init_bias(
            nn.Parameter(torch.Tensor(n_head, head_dim)))
    ...
    def forward(self, x, r):
        ...
        r = self.embed_lin(r).view(bsz, windows, seqlen, self.nh,
                                   self.h).repeat_interleave(n_reps, 1)

        k = F.pad(k, (0, ) * (ndim * 2 - 2) + (self.w, ) * 2).unfold(
            1, seqlen, 1)
        v = F.pad(v, (0, ) * (ndim * 2 - 2) + (self.w, ) * 2).unfold(
            1, seqlen, 1)
        ...
        mask = F.pad(
            mask, (0, ) * (ndim * 2 - 6) + (self.w, ) * 2, value=True).unfold(
                0, seqlen, 1)
...
class MultiDimWindowTransformerLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = MultiDimWindowAttention(
            hidden_dim,
            head_dim,
            n_head,
            hidden_dim,
        ...
        self.ff = nn.Sequential(
            nn.Linear(hidden_dim, ff_dim), act, nn.Linear(ff_dim, hidden_dim))
