...
class LocalAttention(nn.Module):
    def __init__(self,
        ...
        assert not (causal
                    and look_forward > 0), 'you cannot look forward if causal'
    ...
    def forward(self, q, k, v, input_mask=None):
        ...
        (window_size, causal, look_backward, look_forward,
         shared_qk) = (self.window_size, self.causal, self.look_backward,
                       self.look_forward, self.shared_qk)
        b, t, e, device, dtype = *q.shape, q.device, q.dtype
        assert (t % window_size) == 0, (f(
            'sequence length {t} must be divisible by '
            f'window size {window_size} for local attention'))
